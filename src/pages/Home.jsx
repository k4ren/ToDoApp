import { Box, Checkbox, Chip, Divider, Grid, IconButton, List, ListItem, ListItemText, Typography } from "@mui/material";
...
export default function Home() {
  ...
  const handleToggle = (todoId, todo) => {
    axios.put(`${apiUrl}/tareas/${todoId}`, {
      titulo: todo.titulo,
  ...
  return <>
    ...
    <Box sx={{ flexGrow: 1, display: "grid", gap: 2 }}>
      <List>
        {todos.map((todo) => (
          <Box key={todo.id} sx={{ height: 100 }}>
            <ListItem sx={{ mt: 2 }}>
              <Checkbox
                checked={todo.estado}
                onChange={() => handleToggle(todo.id, todo)}
              />
              <ListItemText primary={todo.titulo} secondary={todo.descripcion} style={{ textDecoration: todo.estado ? "line-through" : "none" }} />
              <Chip
                label={todo.prioridad.nombre}
                style={{ backgroundColor: getPriorityColor(todo.prioridad.nombre), color: "white" }}
              />
